...
import { countryMeta, stories, type Country } from '../data'
...
export default function TreatmentCategory() {
  ...
  const catStories = stories.filter((s) => cat.storyMatch !== '___none___' && s.procedure.includes(cat.storyMatch))
        ...
        <div className="uw-grid-3" style={css('display:grid; grid-template-columns:repeat(3,1fr); gap:16px;')}>
          {cat.procedures.map((p) => (
            <El
              as="button"
              key={p.slug}
              onClick={() => openFunnel(cat.name)}
              style={css('text-align:left; border:1px solid #E1E8F7; background:#fff; border-radius:16px; padding:20px; cursor:pointer; font-family:inherit; transition:transform .16s, box-shadow .16s;')}
              hover={css('transform:translateY(-3px); box-shadow:0 16px 34px rgba(35,51,47,.1);')}
            >
              <div style={css('display:flex; align-items:center; justify-content:space-between; margin-bottom:8px;')}>
                ...
                <span style={css('font-size:12.5px; font-weight:700; color:#2B50E4; background:#E7ECFF; padding:4px 9px; border-radius:8px;')}>from {p.priceFrom}</span>
              </div>
              <p style={css('margin:0; font-size:13.5px; color:#7A85A0; line-height:1.5;')}>{p.blurb}</p>
              <span style={css('display:inline-block; margin-top:12px; font-size:13px; font-weight:700; color:#2B50E4;')}>Get a quote →</span>
            </El>
          ))}
        </div>
      </section>
      ...
      <section style={css('background:#E9F0FF;')}>
        <div style={css('max-width:1240px; margin:0 auto; padding:56px 32px;')}>
          ...
          <div style={css('background:#fff; border:1px solid #E1E8F7; border-radius:20px; overflow:hidden; box-shadow:0 14px 34px rgba(35,51,47,.08);')}>
            <div style={css('display:grid; grid-template-columns:1.6fr 1fr 1fr .8fr; padding:15px 22px; background:#16214A; color:#fff; font-size:12.5px; font-weight:700; letter-spacing:.03em;')}>
            ...
            {cat.priceTable.map((r) => {
              ...
              return (
                <div key={r.name} style={css('display:grid; grid-template-columns:1.6fr 1fr 1fr .8fr; padding:16px 22px; border-bottom:1px solid #ECF1FD; align-items:center;')}>
        ...
        <section style={css('background:linear-gradient(140deg,#1C2E7C 0%,#0F1B4A 100%); color:#fff;')}>
          <div style={css('max-width:1240px; margin:0 auto; padding:56px 32px;')}>
            ...
            <div className="uw-grid-3" style={css('display:grid; grid-template-columns:repeat(3,1fr); gap:20px;')}>
              {catStories.map((s) => (
                <div key={s.name} style={css('background:rgba(255,255,255,.05); border:1px solid rgba(255,255,255,.1); border-radius:20px; padding:22px;')}>
